...
class DistributedTransactionTest extends AbstractTransactionTest {
        ...
        transfer =
            context.execute(
                txContext -> {
                  ...
                  final int v0 = txContext.read(new ItemReference(0, 0), data -> data.getInt(0));
                  log.debug("read(0) {}", v0);
                  if (v0 == 0) {
                    return -1;
                  }
                  ...
                  final int v1 = txContext.read(new ItemReference(1, 0), data -> data.getInt(0));
                  log.debug("read(1) {}", v1);
                  txContext.write(
                      new ItemReference(1, 0),
                      (DataOperation<Void>)
                          data -> {
                            data.putInt(0, v1 + v);
                            return null;
  ...
  private void verifyData() {
    ...
    assertEquals(
        (Integer) 0,
        context.execute(
            txContext -> txContext.read(new ItemReference(0, 0), data -> data.getInt(0))));
    assertEquals(
        (Integer) INITIAL_COUNT,
        context.execute(
            txContext -> txContext.read(new ItemReference(1, 0), data -> data.getInt(0))));
  }
}
