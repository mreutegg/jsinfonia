...
class GroupMemoryNodeTransactionIT {

  private static final int NUM_INCREMENTS = 1000;

  @Test
  void singleMember() throws Exception {
    performTest(1);
  }

  @Test
  void replicatedMemoryNode() throws Exception {
    performTest(3);
  }
  ...
  private void performTest(int numMemoryNodes) throws Exception {
    ...
    assertEquals(
        memoryNodes.get(0).getInfo().getAddressSpace(), client.getInfo().getAddressSpace());
    ...
    for (int i = 0; i < NUM_INCREMENTS; i++) {
      txManager.execute(
          (Transaction<Void>)
              txContext -> {
                final int v = txContext.read(new ItemReference(0, 0), ByteBuffer::getInt);
                // System.out.println("read value: " + v);
                txContext.write(
                    new ItemReference(0, 0),
                    (DataOperation<Void>)
                        data -> {
                          data.putInt(v + 1);
                          return null;
    ...
    System.out.println(
        NUM_INCREMENTS + " transactions performed in " + time + " ms (" + txPerSecond + " tx/s).");
    ...
    for (MemoryNode memoryNode : memoryNodes) {
      ...
      while (memoryNode.executeAndPrepare(mt, mt.getMemoryNodeIds()).getVote() != Vote.OK) {
        // execute again
      }
