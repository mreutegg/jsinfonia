...
/**
 * The <code>ItemManager</code> keeps track of items that are currently in use by an application. It
 * is the application responsibility to call {@link #alloc()} and {@link #free(ItemReference)}
 * accordingly.
 *
 * <p>The <code>ItemManager</code> maintains an in-use <code>BitSet</code> on every <code>MemoryNode
 * </code>. Allocation of a new <code>Bucket</code> is done transactionally by setting the
 * respective bit in the in-use <code>BitSet</code>. Similarly, {@link #free(ItemReference)} will
...
public class ItemManagerImpl implements ItemManager {
  ...
  /**
   * Creates a new <code>ItemManager</code> instance based on existing data identified by the given
   * <code>headerRef</code>.
   *
  ...
  private ItemManagerImpl(
      TransactionContext txContext, ItemReference headerRef, FixedBitSet allocations) {
   ...
   */
  public static ItemReference initialize(
      TransactionContext txContext, int memoryNodeId, int addressSpace) {
