...
public class SimpleApplicationNode implements ApplicationNode {
  ...
  public SimpleApplicationNode(
      MemoryNodeDirectory<? extends MemoryNode> directory, ExecutorService executor) {
  ...
  public Response executeTransaction(final MiniTransaction tx) {
    ...
    for (final Integer memoryNodeId : memoryNodeIds) {
      ...
      callables.add(
          () -> directory.getMemoryNode(memoryNodeId).executeAndPrepare(mt, memoryNodeIds));
    }
