 ...
 *
 * <p>A SinfoniaList is initialized with a header reference with the following structure:
 *
 * <pre>
 * +---------------------------------------------------------------------------+
 * | version |         |         |        |            size (4 byte)           |
 ...
 * </pre>
 *
 * The first byte contains a version tag. At offset 4, the size of the list is stored. At offset 8
 * and 16 the head and the tail buckets are referenced.
 *
 * <p>Each linked bucket has the following structure:
 *
 * <pre>
 * +---------------------------------------------------------------------------+
 ...
 * </pre>
 *
 * The next item reference either points to the next bucket if there are more items in the list or
 * points to the header item if it is the tail bucket in the linked list. That is, the next
 * reference is equal to {@link #headerRef}.
 *
...
public class SinfoniaList<E> extends AbstractList<E> {
  ...
  public static <E> List<E> newList(
    ...
    return new SinfoniaList<>(headerRef, txContext, factory, reader, writer);
  }
  ...
  /**
   * Adds the given element to the tail bucket if there is still space left in the bucket and
   * increments the size of the list. Also returns <code>false</code> if there is no bucket yet.
   *
  ...
  private boolean addInternal(E e) {
    ...
    if (Iterables.size(entries) == num) {
      txContext.write(
          headerRef,
          data -> {
            data.putInt(HEADER_OFFSET_SIZE, data.getInt(HEADER_OFFSET_SIZE) + 1);
            return null;
          });
      return true;
    }
