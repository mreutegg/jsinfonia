...
public class Utils {
  ...
  public static MiniTransaction convert(TMiniTransaction tx) {
    ...
    if (tx.isSetCompareItems()) {
      for (TItem item : tx.getCompareItems()) {
        ...
        mt.addCompareItem(
            new Item(ref.getMemoryNodeId(), ref.getAddress(), ref.getOffset(), item.getData()));
      }
    }
    if (tx.isSetReadItems()) {
      for (TItemReference ref : tx.getReadItems()) {
        byte[] data = new byte[ref.getSize()];
        mt.addReadItem(new Item(ref.getMemoryNodeId(), ref.getAddress(), ref.getOffset(), data));
      }
    }
      ...
      case BAD_CMP:
        ...
        if (result.isSetFailedCompares()) {
          for (TItemReference failedRef : result.getFailedCompares()) {
            refs.add(new ItemReference(failedRef.getMemoryNodeId(), failedRef.getAddress()));
          }
        }
        r = new Result(refs);
        break;
    ...
    if (response.isSuccess()) {
      ...
      if (response.isSetFailedCompares()) {
        for (TItemReference ref : response.getFailedCompares()) {
          failedCompares.add(new ItemReference(ref.getMemoryNodeId(), ref.getAddress()));
        }
      }
  ...
  public static ItemReference convert(TItemReference reference) {
    return new ItemReference(reference.getMemoryNodeId(), reference.getAddress());
  }
}
