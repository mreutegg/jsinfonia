...
public class FileRedoLog implements RedoLog, Closeable {
  ...
  private final BlockingQueue<SettableFuture<Boolean>> syncQueue = new LinkedBlockingQueue<>();
  ...
  public void append(String txId, List<Item> writeItems, Set<Integer> memoryNodeIds)
    ...
    for (Item item : writeItems) {
      ...
      buffers.add(meta);
      WriteItem wi =
          new WriteItem(item.getAddress(), item.getOffset(), writeSize, data.remaining());
      buffers.add(data);
      ...
      if (commit) {
        synchronized (readFile) {
          ...
          for (WriteItem wi : txInfo.writeItems) {
            ...
            memoryNode.applyWrite(readChannel, wi.address, wi.offset, wi.length);
          }
        }
      }
    } catch (IOException e) {
