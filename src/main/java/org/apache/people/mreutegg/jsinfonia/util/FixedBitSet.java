...
/**
 * This class is inspired by Apache Lucene FixedBitSet but data is located in memory node items.
 *
 * <p>The header item has the following structure:
 *
 * <pre>
 * +---------------------------------------------------------------------------+
...
public class FixedBitSet {
  ...
  /**
   * Creates a <code>FixedBitSet</code> based on existing data. The given <code>ItemReference</code>
   * identifies the header item.
   *
  ...
  /**
   * Creates a <code>FixedBitSet</code> and initializes the header item with the passed <code>
   * dataItemRefs</code> to store the actual bits.
   *
   * @param context the transaction context to access items.
   * @param headerRef the reference to the header item.
   * @param dataItemRefs the references to the data items that store the actual bits. FixedBitSet
   *     supports up to <code>Character.MAX_VALUE</code> data item references.
   */
  public FixedBitSet(
      TransactionContext context, ItemReference headerRef, final List<ItemReference> dataItemRefs) {
   ...
   * @param dataItemRefs the references to the data items that store the actual bits. FixedBitSet
   *     supports up to <code>Character.MAX_VALUE</code> data item references.
   * @param length limits the length of this bit set to the given <code>length</code>.
   */
  public FixedBitSet(
      TransactionContext context,
      ItemReference headerRef,
      final List<ItemReference> dataItemRefs,
      final int length) {
    this.context = context;
    this.headerRef = headerRef;
    if (dataItemRefs.size() > Character.MAX_VALUE) {
      throw new IllegalArgumentException(
          "FixedBitSet only supports up to " + (int) Character.MAX_VALUE + " dataItemRefs");
    }
    ...
    if (dataItemRefs.size() * 12 > dataSize - META_LENGTH) {
      throw new IllegalArgumentException(
          "Not enough space in header item ("
              + (dataSize - META_LENGTH)
              + ") to store "
              + dataItemRefs.size()
              + " references.");
    }
    ...
    if (length > maxWords * 64) {
      throw new IllegalArgumentException(
          "length limit > capacity: " + length + " > " + (maxWords * 64));
    }
  ...
  /**
   * Enforces length of this bit set. If the given <code>index</code> is greater than the length,
   * then this method returns -1. Otherwise the index is returned.
   *
  ...
  private BitsReference getBitsReference(final int index) {
    return context.read(
        ...
        data -> {
          ...
          // log.debug("BitsReference for index " + index + ": itemIndex" + itemIndex + ", subIndex:
          // " + subIndex);
